...
Допущения: 
    ...
    2) Отсутствие сверхдисперсии (далее проверяем);
...
{glm_AE %>% broom::tidy(exponentiate = TRUE,
  ...
  flextable()} %>% 
  add_header_row(., values = "Частота НЯ в гр. терапии Климаксаном по отношению к гр. терапии Анжеликом", colwidth = length(.$col_keys)) %>% 
  flex_default_fun()
```
