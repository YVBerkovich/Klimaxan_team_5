...
# Раздел 1. Анализ первичной конечной точки
...
# Создание графика
forest_plot_pois <- ggplot(plot_pois_df, aes(x = Estimate, y = Comparison)) +
  geom_vline(xintercept = 1, linetype = "dashed", color = "red", size = 1) +
  geom_point(size = 4) +
  geom_errorbarh(aes(xmin = CI_lower, xmax = CI_upper), 
                 height = 0.1, size = 1, color = "blue") +
  annotate("text", x = 1.1, y = 1.3, 
           label = sprintf("IRR: %.2f\n95%% ДИ: [%.2f; %.2f]\nP = %.1f", 
                          1.04, 0.96, 1.13, 0.3),
           hjust = 0.5, vjust = 0.5, color = "black") +
  scale_x_continuous(breaks = seq(0,5, 0.02),
                     name = "Отношение частот, IRR (Климаксан / Анжелик)") +
  labs(title = "Анализ безопасности",
       subtitle = "Отношение частоты НЯ (IRR)\nв группе Климаксан\nпо отношению к Анжелик",
       y = "") +
  theme_bw() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
